...
function clickFunc(point) {
  if (cnt < 12) {
    questionBox.textContent = QuestionData[cnt].title;
    type01.textContent = QuestionData[cnt].answera;
    type02.textContent = QuestionData[cnt].answerb;
    bar.style.width = (cnt / 12) * 100 + "%";

    const newScore = score.map(function (item, idx) {
      if (item.id === QuestionData[cnt].type) {
    ...
    const mbti = score.reduce(function (acc, cur) {
      if (cur.num >= 2) {
        return acc + cur.id.charAt(0);
      } else {
        return acc + cur.id.charAt(1);
      }
    }, "");
    ...
    cat.src = `./images/${selectedData.image}`;
